...
def compute_length_of_longitude(degree_lat):
    assert degree_lat >= -90 and degree_lat <= 90, 'invalid latitude value'
...
def get_area_for_lat(lat, lat_frac = 0.5, lon_frac = 0.5):
    assert lat >= -90 and lat <= 90, 'invalid latitude value'
    assert lat_frac > 0 and lon_frac > 0, 'lon and lat fractions must be positive'
...
def get_area_weighted_sum_per_year(lon_lat_dataset, variable_name, lat_frac=0.5, lon_frac=0.5):
    ...
    areas = lon_lat_dataset.index.to_frame()['Lat'].map(lambda x: get_area_for_lat(x, lon_frac=lon_frac, lat_frac=lat_frac))
    total_values_per_gridcell = areas * lon_lat_dataset[variable_name]

    return total_values_per_gridcell.groupby('Year').sum().rename('value')

def get_area_weighted_mean_per_year(lon_lat_dataset, variable_name, lat_frac=0.5, lon_frac=0.5):
    assert lon_lat_dataset.index.names == ['Lon', 'Lat', 'Year'], "Index does not contain Lon, Lat, Year"
    assert lon_lat_dataset.index.is_unique, "Index is not unique"
    assert np.all(~np.isnan(lon_lat_dataset)), "NaNs in dataset!"
    ...
    return (total_values_per_gridcell.groupby('Year').sum() / areas.groupby('Year').sum()).rename('value')
