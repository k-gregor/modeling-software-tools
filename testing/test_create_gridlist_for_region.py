...
def test_create_gridlist_for_region_gets_climate_coordinates_within_region():
    ...
    _, gridlist_points_in_germany, _ = get_gridlist_from_climate_data_and_region('testing/dummy_temp.nc', ['Germany'], 'admin_0_countries')
    assert len(gridlist_points_in_germany) == 1

    _, gridlist_points_in_france_and_germany, _ = get_gridlist_from_climate_data_and_region('testing/dummy_temp.nc', ['France', 'Germany'], 'admin_0_countries')
    assert len(gridlist_points_in_france_and_germany) == 1

    _, gridlist_points_in_france, _ = get_gridlist_from_climate_data_and_region('testing/dummy_temp.nc', ['France'], 'admin_0_countries')
